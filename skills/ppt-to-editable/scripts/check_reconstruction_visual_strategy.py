...
TEXTLESS_STRATEGIES = {"source_textless_crop", "local_textless_crop", "textless_crop"}
SOURCE_CROP_STRATEGIES = TEXTLESS_STRATEGIES | {"source_icon_crop", "tight_source_crop"}
NATIVE_STRATEGIES = {"native_redraw", "native_lines", "native_shapes", "native_reconstruction"}
...
def is_textless_picture(element: dict[str, Any] | None) -> bool:
    ...
    return str(source.get("kind", "")).lower() in TEXTLESS_STRATEGIES
...
def check_region(region: dict[str, Any], element_map: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    expected = expected_strategy.lower()
    actual = actual_strategy.lower()
    ...
    high_risk = visual in HIGH_RISK_VISUAL_TYPES
    if high_risk and actual in NATIVE_STRATEGIES and not region.get("override_reason"):
