#!/usr/bin/env python
"""Deck controller for ppt-to-editable v3.0 preview.
...
CONVERSION_MODE_CHOICES = ("single-page-token-saving", "full-deck-convenience")
...
PRIMARY_SINGLE_PAGE_ENGINE_ID = "ppt-to-editable-v3-preview"
...
def context_reference_paths() -> dict[str, str]:
    ...
    return {
        "single_page_engine_skill_3_0": str(engine_root / "SKILL.md"),
...
def reference_loading_policy(required_reading: list[dict]) -> dict:
    return {
        ...
        "principle": (
            "Read the small per-slide contract and only the references named by the "
            "slide job. Do not load old rounds, process audits, or previous test "
            "artifacts as conversion context for a fresh run."
        ),
    }
...
def load_and_validate_gates(
    ...
    if not isinstance(conversion_gate, dict):
        raise ValueError(
            ...
            "choice between single-page-token-saving and full-deck-convenience."
        )
    ...
    if choice == "all":
        ...
        if conversion_mode == "full-deck-convenience" and choice == "sample":
            raise ValueError(
        ...
        "user_confirmation_required": {
            "conversion_mode_gate": {
                "question": (
                    ...
                    "结果：输出一页可编辑 PPT。\n\n"
                    "2. 全量省力模式\n"
                    "适合：你想把整份 image-only PPTX 转成可编辑 deck。\n"
                    "消耗：较高，因为会按页拆开处理。\n"
                ...
                "options": list(CONVERSION_MODE_CHOICES),
                "internal_recording_note": (
                    ...
                    "Use full-deck-convenience for multi-page PPTX conversion."
                ),
            },
            "ocr_runtime_gate": {
            ...
            "scope_and_worker_gate": {
                "question": (
                    ...
                    "2. 只转换指定页\n"
                    "3. 先转换 1 页看看效果\n\n"
                    "说明：转换页数越多，消耗的 token 越多。为了提高质量，"
                    "我会把每一页交给一个独立的转换任务处理；这些任务会读取"
                    "对应页面的图片和文字内容。\n\n"
                    "请告诉我你的选择。"
                ),
    ...
    required_reading = [
        {
            ...
            "path": reference_paths["single_page_engine_skill_3_0"],
            "must_read": True,
            "reason": "Integrated v3 preview single-page and multi-page conversion skill for this run.",
        },
        {
            "id": "single_page_engine_source_crop_tools",
            "path": reference_paths["single_page_engine_source_crop_tools"],
            "must_read": True,
            "reason": "Source crop, textless crop, and clean badge composite rules.",
        },
        {
            "id": "single_page_engine_reconstruction_qa",
            "path": reference_paths["single_page_engine_reconstruction_qa"],
            "must_read": True,
            "reason": "Reconstruction QA gates and required deterministic QA flow.",
        },
        {
            "id": "single_page_engine_reconstruction_plan_schema",
            "path": reference_paths["single_page_engine_reconstruction_plan_schema"],
            "must_read": True,
            "reason": "source_reconstruction_plan.json schema used by the deterministic packager.",
        },
        {
            "id": "single_page_engine_ocr_to_plan_review",
            "path": reference_paths["single_page_engine_ocr_to_plan_review"],
            "must_read": True,
            "reason": "Rule that OCR scaffolds require review before packaging.",
        },
        {
        ...
        },
        {
            ...
            "must_read": True,
            "reason": "User-accepted visual fidelity preferences for crop/rebuild/text QA.",
        },
        {
            "id": "multi_agent_slide_protocol",
            "path": reference_paths["multi_agent_slide_protocol"],
            "must_read": True,
            "reason": "Multi-Agent progress, OCR runtime, draft status, and QA handoff rules.",
        },
        {
            "id": "visual_fidelity_execution",
            "path": reference_paths["visual_fidelity_execution"],
            "must_read": True,
            "reason": "Execution-time rules for PowerPoint render loop, Microsoft YaHei, effect clearing, and region crops.",
        },
    ]
    manifest = {
        ...
        "worker_contract": {
            ...
            "controller_agent_must_not_generate_slide_pptx": True,
        ...
        "reference_loading_policy": reference_loading_policy(required_reading),
        "quality_rules": {
            ...
            "must_use_single_page_engine_3_0": True,
...
Use the bundled `ppt-to-editable` v3 preview single-page engine on exactly one source image.
...
- Read every file in **Required Reading** before conversion work.
- Run this task in a real separate per-slide worker context. The controller/test thread must not build this slide itself.
...
- Use this `ppt-to-editable` v3 preview package as the primary single-page conversion engine.
...
- Use crop-first judgment for complex artwork regions: product photos, detailed icons, curved paths, gradient arrows, textured modules, and source-specific card stacks should be source-derived crops or textless crops unless a native redraw clearly matches the original.
...
If any required reference is missing or unreadable, stop and write `qa_summary.json` with `status=failed-retryable`.
...
- If the slide is reconstruction or mixed reconstruction, write/review `source_reconstruction_plan.json`, then run `run_reconstruction_qa.py` or `package_reconstruction_deck.py` from this v3 preview package.
...
- In `qa_summary.json`, set `single_page_engine_used` to `{PRIMARY_SINGLE_PAGE_ENGINE_ID}` and list every integrated 3.0 script actually called in `single_page_engine_scripts_used`.
...
For complex structured pages, record a semantic 3-5 region crop/native/text plan in `region_crop_plan`; do not use one whole-slide textless crop unless explicitly accepted as fallback.
...
Record `complex_visual_strategy_review` and explain which complex regions used crop-first handling. A success status is not allowed when complex artwork was redrawn natively and became visibly worse.
...
After the single-page QA chain writes its own `qa_summary.json`, run this 3.0 handoff command to convert it into the controller-required summary. Replace the worker id value with your real agent id or thread id; do not invent one:

```text
python "{handoff_script}" --slide-job-manifest "{Path(manifest['output_dir']).parent / 'slide_job_manifest.json'}" --attempt-dir "{manifest['output_dir']}" --worker-agent-id "<your real worker_agent_id>" --output "{manifest['output_dir']}\\qa_summary.json"
```

This command preserves the original single-page summary as `qa_summary.single-page.json`, writes the full 3.0 `qa_summary.json`, normalizes render reports for the 3.0 controller, and appends `qa_done` to `progress.json`.
...
def validate_worker_identity(summary: dict, slide_index: int) -> tuple[bool, str | None, str | None]:
    ...
    if execution_model and execution_model != "delegated-real-per-slide-worker":
...
def collect_slide_result(slide_info: dict, run_dir: Path) -> dict:
    ...
    if qa_summary.exists():
        ...
        if missing_fields:
            ...
            limitations.append(
                "Controller rejected the slide output because qa_summary.json did not report all required v3 preview fields."
            )
            result["limitations"] = limitations
        ...
        if status_for_policy in ("passed", "accepted-with-limitations"):
            ...
            if not engine_ok:
                mark_result_failed(
                    ...
                    "Controller rejected the slide output because it did not prove use of the bundled v3 preview single-page engine and deterministic packaging scripts.",
                )

        status_for_policy = result["status"]
        if status_for_policy in ("passed", "accepted-with-limitations"):
            worker_ok, worker_error, worker_key = validate_worker_identity(summary, slide_index)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ppt-to-editable v3 preview deck controller")
