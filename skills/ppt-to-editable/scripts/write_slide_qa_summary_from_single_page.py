...
def region_crop_plan(plan: dict, crop_report: dict | None) -> list[dict]:
    ...
    if crop_outputs:
        regions.append(
            {
                "id": "source-crops",
                "strategy": "tight-source-crops",
                "crop_count": len(crop_outputs),
            }
        )
    while len(regions) < 3:
        regions.append({"id": f"region-{len(regions) + 1}", "strategy": "not-reported"})
    return regions
...
def complex_visual_strategy_review(plan: dict, visual_report: dict | None) -> dict:
    ...
    complex_regions = [
        region for region in regions if str(region.get("visual_type", "")).lower() in HIGH_RISK_VISUAL_TYPES
    ]
    crop_first_regions = [
        region for region in complex_regions if str(region.get("actual_strategy", "")).lower() in CROP_FIRST_STRATEGIES
    ]
    native_complex_regions = [
        region
        for region in complex_regions
        if str(region.get("actual_strategy", "")).lower() in NATIVE_STRATEGIES
        and not region.get("override_reason")
    ]
    visual_warnings = []
        ...
        history.append(
            {
                ...
                "current_step": "3.0 handoff qa_summary generated from integrated single-page output",
