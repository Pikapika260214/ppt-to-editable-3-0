...
For ppt-to-editable v3 preview multi-page and delegated per-slide Agent work, also read `visual-fidelity-execution.md` before conversion. That file contains execution-time rules for PowerPoint rendered PNG review, Microsoft YaHei font enforcement, clearing unwanted shape effects, and 3-5 region textless crop planning.
...
- Preserve semantic text groups. If the source has one multi-line paragraph or label block, keep it as one text box unless line-level fidelity clearly requires otherwise.
