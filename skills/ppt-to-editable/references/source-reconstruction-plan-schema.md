...
- `native_reconstruction`: simple native shapes or lines that preserve the source.
...
- `native_redraw`: only acceptable for high-risk regions when an explicit `override_reason` explains why native redraw is visually faithful.

For high-risk visual regions, choose crop-first by default. Native redraw needs an explicit `override_reason` and must still pass rendered PNG review. If a complex region looks like a generic PowerPoint template after redraw, change it to `source_textless_crop` plus editable text.
