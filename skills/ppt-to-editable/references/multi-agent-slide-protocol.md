...
Before doing any conversion work, read:

1. This `ppt-to-editable` v3 preview `SKILL.md`.
2. The v3 preview references required by the slide job manifest, especially source crop tools, reconstruction QA, reconstruction plan schema, and OCR scaffold review.
3. The slide job manifest.
4. `visual-fidelity-contract.md`.
5. `visual-fidelity-execution.md`.
6. This protocol.
7. `page_conversion_contract.json`, if listed in the slide job manifest.
8. `crop_manifest_contract.json`, if listed in the slide job manifest.

Record these files in `qa_summary.json` under `references_read`.
...
After the single-page QA chain writes its own `qa_summary.json`, run the 3.0 handoff
...
This helper converts the single-page QA output into the full 3.0 controller
