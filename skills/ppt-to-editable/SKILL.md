...
This is the v3.0 preview release candidate for the `ppt-to-editable` skill. It is designed to replace the legacy single-page-only skill after validation, while keeping the same public skill name.

Version 3.0 integrates the proven single-page conversion scripts into this skill folder and adds deck-level control for multi-page image-only PPTX inputs. A valid multi-page run should use this folder's own `scripts/` and `references/`, not a separate local development folder.
...
如果你更想省 token，建议先选单页省 token 模式。如果你更想省操作，可以选全量省力模式。
...
- `full-deck-convenience`: multi-page PPTX conversion, all pages or selected pages.
...
If `conversion_mode_gate.user_choice` is `full-deck-convenience`, ask one combined question before creating slide jobs:
...
说明：转换页数越多，消耗的 token 越多。为了提高质量，我会把每一页交给一个独立的转换任务处理；这些任务会读取对应页面的图片和文字内容。
...
- whether each page may be read by a separate conversion task for this conversion.
...
Keep context small. For a fresh run, read the slide job, the two small contracts, and the required v3 references. Do not read historical test reports, old round outputs, prior visual QA HTML, or baseline artifacts unless the user explicitly asks to reuse them for diagnosis.

## 3.0 Multi-Page Workflow
...
4. Apply the Scope + Worker Gate according to the selected mode and write `scope_and_worker_gate` into `gates.json`. In single-page mode, select exactly one sample slide. In full-deck mode, ask whether to convert all pages or selected pages. Do not start deck initialization or worker dispatch before the user chooses page scope and acknowledges worker/token/content-sharing implications.
...
8. Dispatch each slide job to a real separate per-slide worker. The controller/test thread must not build multiple slide outputs itself.
9. Each worker reads the generated `AGENT_TASK.md`, this `SKILL.md`, and the required references, then processes exactly one source PNG using this skill's integrated single-page scripts.
10. Each worker runs the appropriate deterministic single-page chain, normally `run_reconstruction_qa.py` for reconstruction/mixed reconstruction, plus OCR, crop, editability, and PowerPoint render QA scripts as needed.
11. Each worker runs `scripts/write_slide_qa_summary_from_single_page.py` to convert the single-page QA output into the deck-controller `qa_summary.json` contract.
...
For quality tests, successful pages must report `single_page_engine_used: "ppt-to-editable-v3-preview"` and list the v3 scripts actually called. A page that calls any external development folder as its primary engine should be marked `failed-retryable`, not accepted.
...
- In v3.0 preview, use `scripts/extract_source_crops.py` and `scripts/make_textless_crops.py` for reviewed crop manifests before writing one-off crop code.
...
- Product photos, detailed icons, curved paths, gradient arrows, textured modules, layered card stacks, and source-specific process visuals use crop-first judgment; native redraw needs rendered evidence that it matches the source.
...
Treat `native_redraw_used_for_high_risk_region`, `expected_textless_crop_missing`, and `textless_crop_text_overlay_missing` as review gates. Fix the strategy review, add the missing textless crop, or record an explicit override reason before packaging.
...
- high-risk visual modules have a passing `visual_strategy_report.json` or an explicit recorded override;
