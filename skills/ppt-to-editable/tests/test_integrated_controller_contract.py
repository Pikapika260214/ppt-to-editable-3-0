...
class OcrRuntimeContractTests(unittest.TestCase):
    ...
    def test_probe_deck_returns_read_only_gate_template(self) -> None:
        ...
        self.assertIn("全量省力模式", mode_question)
        self.assertIn("发单张 PNG", mode_question)
        self.assertIn("整份 image-only PPTX", mode_question)
        self.assertIn("省 token", mode_question)
        self.assertIn("省操作", mode_question)
        for forbidden in (
            "app-native",
            "external-codex",
            "worker runtime",
        ...
        self.assertIn("独立的转换任务", scope_question)
        ...
        write_json(
            attempt_dir / "qa_summary.json",
            {
                ...
                **visual_fidelity_review_fields(),
                ...
                "region_crop_plan": ["title", "main", "footer"],
    ...
    def test_manifest_requires_worker_to_report_ocr_runtime_status(self) -> None:
        ...
        self.assertEqual(manifest["worker_contract"]["reuse_skill"], "ppt-to-editable-v3-preview")
        required_ids = {item["id"] for item in manifest["required_reading"] if item["must_read"]}
        self.assertIn("single_page_engine_skill_3_0", required_ids)
        self.assertIn("single_page_engine_reconstruction_qa", required_ids)
        self.assertIn("single_page_engine_source_crop_tools", required_ids)
    ...
    def test_collect_slide_result_auto_closes_raw_single_page_summary(self) -> None:
        ...
        write_json(
            attempt_dir / "source_reconstruction_plan.json",
            {
                ...
                "strategy_review": {
                    "regions": [
                        {"id": "header", "actual_strategy": "native_reconstruction"},
                        {"id": "body", "actual_strategy": "textless_crop_plus_editable_text"},
                        {"id": "footer", "actual_strategy": "native_reconstruction"},
                    ]
                },
            },
        )
        ...
        self.assertEqual(summary["single_page_engine_used"], "ppt-to-editable-v3-preview")
